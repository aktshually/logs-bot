...
class onMemberJoin(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        if channel_id != 'N':
            ...
            embed = discord.Embed(
                description = data[f"{lang_json}"]["events"]["onMemberJoin"]["Message"].replace("{user}", f"{member.mention}"),
                color = discord.Colour.green()
            )
            embed.set_author(name = data[f"{lang_json}"]["events"]["onMemberJoin"]["Author"].replace("{user}", f"{member.mention}"), icon_url = member.guild.icon_url)
